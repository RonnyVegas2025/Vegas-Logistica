...
export default async function EntregadoresPage() {
  ...
  const { data: entregadores } = await sb
    .from('entregadores').select('*, filiais(nome), parceiros(nome)').order('nome')
  ...
  return (
    <div className="fade-in">
      <div className="page-h">
        ...
        <EntregadorModal action={createEntregador} filiais={filiais??[]} parceiros={parceiros??[]}/>
      </div>
      <div className="card">
        ...
        <div className="overflow-x-auto">
          <table className="tbl w-full">
            <thead>
              <tr>
                <th>Nome</th><th>Filial</th><th>Parceiro</th>
                <th>Telefone</th><th>Formas habilitadas</th><th>Status</th><th>Ações</th>
              </tr>
            </thead>
            <tbody>
              {(entregadores??[]).map(e=>(
                <tr key={e.id}>
                  ...
                  <td className="flex items-center gap-2">
                    <EntregadorModal entregador={e} action={updateEntregador} filiais={filiais??[]} parceiros={parceiros??[]}/>
              ...
              {!entregadores?.length&&(
                <tr><td colSpan={7} className="text-center text-gray-400 py-10">Nenhum entregador cadastrado</td></tr>
              )}
            </tbody>
          </table>
        </div>
